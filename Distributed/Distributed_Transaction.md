...
通过上面的阐述，在分布式环境下，要达到ACID是相对困难的。于是有人提出了CAP原则，这个原则相对于ACID而言，宽松了不少。

CAP原则，又叫做CAP定理，指在一个分布式系统中，一致性（Consistency）、可用性（Availability）、分区容错性（Partition Tolerance）三者不可兼得。
