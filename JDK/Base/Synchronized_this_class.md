...
当 synchronized 修饰普通方法时，被修饰的方法被称为同步方法，其作用范围是整个方法，**作用的对象是调用这个方法的<font color='red'>对象（就是类的实例，同一个类的不同的实例，具有不同的锁）</font>**。
...
从上述结果可以看出，这 5 个线程共享的是**<font color='red'>同一把锁</font>**。
...
从上述结果可以看出，虽然是不同的对象，但它们使用的**<font color='red'>仍然是同一把锁</font>**。
