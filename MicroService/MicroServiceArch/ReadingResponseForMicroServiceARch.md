...
但这样，事件累积过多，会有性能问题，所以**可以对一部分历史数据进行计算，得到一个中间的快照，之后的计算在快照的基础上再叠加**。（解决性能的办法之一）
...
**读服务可以订阅写服务的 Domain Event，也可以是 MySQL 的 binlog**（用于同步读服务器的数据）。
...
开源的 API Gateway 大多不支持 API 数据组合的功能，所以公司内的API Gateway 有时候有两层：一层是 nginx 之类的负责简单路由和鉴权的gateway，后面还有一个业务的 BFF（Backend For Front） 来负责拼装端上需要的数据。
...
所以，如果使用JSON Graph 来描述内部 API 提供的数据，然后用 JS 来定制图上的查询，就能解决RESTful的缺点。这就是大家比较熟悉的 GraphQL：
