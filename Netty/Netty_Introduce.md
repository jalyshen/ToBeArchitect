...
​        普遍的认为 Netty 的性能比 Tomcat 高。其实不然，tomcat 从 6.x 开始就支持了 nio 模式，并且后续还有 APR 模式-- 一种通过 jni 调用 apache 网络库的模式，相比于就的 bio 模式，并发性能得到了很大提高，特别是 APR 模式，而 netty 是否比 tomcat 性能更高，更取决于 netty 程序员的技术实力。
...
​        当一个连接建立之后，它有两个步骤要做，第一步，是接收玩客户端发来的全部数据；第二步，是服务端处理完请求业务之后返回 reponse 给客户端。NIO 和 BIO 的区别主要是在第一步。
