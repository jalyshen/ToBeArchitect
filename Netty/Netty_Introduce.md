...
​        大家悉知，Java 的内存有堆内存、栈内存和字符串常量池等等。其中，堆内存是占用内存空间最大的一块，也是 Java 对象存放的地方。一般的，如果需要从 IO 读取数据到堆内存，中间需要经过 Socket 缓冲区，也就是说，一个数据会被拷贝两次才能到达它的目的地。如果数据量大，就会造成不必要的资源浪费。
